...
var src_default = new import_plugin.Transformer({
  ...
  async transform({ asset, config, logger }) {
    ...
    for (const options of sequentialOptions) {
      ...
      const renderedOptions = renderOptions({ ...(0, import_omit.default)(["from", "to", "read", "write"], options), from: reader, to: writer, log: logFile.name });
      const command = ["pandoc", ...renderedOptions].join(" ");
      logger.verbose({ message: `Running '${command}'` });
      const output = (0, import_child_process.execSync)(command, { input, cwd: config.root, encoding: "utf-8" });
      logger.verbose({ message: `Done` });
      asset.setCode(output);
      const logEntries = JSON.parse(import_fs.default.readFileSync(logFile.name));
